...
#'
compute_value <- function(nuts = "Italia", h = 3, last_yr, corine_code) {

  if ("2.1.2" %in% corine_code)
    stop("The Corine 3 class '2.1.2' (Permanently irrigated land) cannot be selected. Please use '2.1.1' (non-irrigated arable land) instead.")
  ...
  last_yr_dfl <- gdp_defl[gdp_defl$original_period == as.character(last_yr), ]$defl


  if (!is.element("2.1.1", corine_code) | "2.1.1" %in% corine_code & length(corine_code) > 1) {

  metadata <- master_table[master_table$corine3_code %in% corine_code[corine_code != "2.1.1"], # exluding arable land
  ...
  avg_values <- data_raw %>%
    ...
    dplyr::mutate(value = value / defl * last_yr_dfl) %>%
  ...
  if (is.element("2.1.1", corine_code)) {
    ...
    avg_values_arable <- data_raw_arable %>%
      ...
      dplyr::summarise(value = mean(value, na.rm = TRUE)) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(corine3_code = "2.1.1")

  }

  if (length(corine_code) == 1 & "2.1.1" %in% corine_code) {

    value <- avg_values_arable

  } else if (!is.element("2.1.1", corine_code)){
  ...
  # Adjusting factors can be found in the master_table

  adj_fct <- master_table[master_table$corine3_code %in% value$corine3_code, c("corine3_code", "fct")] %>%
    unique()
